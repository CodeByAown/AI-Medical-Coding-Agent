...
import {
  ...
  User, Hash, History, ClipboardCheck, Info,
} from "lucide-react"
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 space-y-6">
      ...
      <div className="text-center space-y-1">
        ...
        <p className="text-sm text-slate-500">Processing clinical text...</p>
      </div>
...
function ResultsPanel({ result, onNewSession }: { result: CodingResult; onNewSession: () => void }) {
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <Button variant="outline" size="sm" onClick={copyToClipboard} className="text-xs">
          <Copy className="h-3 w-3 mr-1" /> Copy codes
        </Button>
      </div>

      {/* Meta row */}
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <Badge variant={result.status === "completed" ? "success" : result.status === "error" ? "danger" : "warning"} className="text-[10px]">
          {result.status === "completed" ? "Completed" : result.status === "error" ? "Error" : "Needs Review"}
        </Badge>
        ...
        {result.codes.length > 0 && (
          <div className="flex items-center gap-1.5 text-xs text-slate-500">
            Avg confidence: <ConfidenceBar value={avgConf} size="sm" showLabel />
          </div>
        )}
      </div>
      ...
      {/* No codes returned */}
      {result.codes.length === 0 && result.status !== "error" && (
        <div className="mb-4 flex items-start gap-2.5 rounded-xl border border-slate-700/50 bg-slate-800/30 px-4 py-3">
          <Info className="h-4 w-4 text-slate-400 flex-shrink-0 mt-0.5" />
          <p className="text-sm text-slate-400">No codes were assigned. The AI may need more clinical detail or this note may require manual review.</p>
        </div>
      )}
      ...
      {/* Post-coding workflow actions */}
      <div className="mt-4 pt-4 border-t border-[#1e2d4d] flex items-center gap-2 flex-shrink-0">
        <Button variant="outline" size="sm" className="flex-1 text-xs" onClick={onNewSession}>
          <Brain className="h-3.5 w-3.5 mr-1" /> New Session
        </Button>
        <Link href="/dashboard/history" className="flex-1">
          <Button variant="outline" size="sm" className="w-full text-xs">
            <History className="h-3.5 w-3.5 mr-1" /> View in History
          </Button>
        </Link>
        {result.requires_human_review && (
          <Link href="/dashboard/review" className="flex-1">
            <Button variant="default" size="sm" className="w-full text-xs bg-amber-600 hover:bg-amber-500">
              <ClipboardCheck className="h-3.5 w-3.5 mr-1" /> Go to Review Queue
            </Button>
          </Link>
        )}
      </div>
...
export default function CodingPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(extractErrorMessage(err, "Coding failed. Please check that the backend is running."))
    } finally {
      ...
      <div className={`flex flex-col border-r border-[#1e2d4d] transition-all duration-300 ${result || isProcessing ? "w-1/2" : "w-full max-w-3xl mx-auto"}`}>
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <p className="text-[10px] text-slate-600 leading-relaxed">
                Controls extraction strategy — e.g., SOAP Notes parse structured sections; Discharge Summaries extract DRG-relevant diagnoses.
              </p>
            </div>
            <div className="space-y-1.5">
              ...
              <p className="text-[10px] text-slate-600 leading-relaxed">
                Applies specialty-specific coding guidelines — e.g., Cardiology enforces HF ejection fraction specificity; Orthopedics adds laterality codes.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            />
            <p className="text-[10px] text-slate-600 text-right">{text.length} characters · min 10 required</p>
          </div>
          ...
          >
            <div className="flex-1 overflow-hidden p-6">
              {isProcessing ? (
                ...
                <ResultsPanel result={result} onNewSession={handleNewSession} />
              ) : null}
            </div>
          </motion.div>
        )}
